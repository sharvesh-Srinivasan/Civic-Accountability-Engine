...
export default function Leaderboard() {
  ...
  return (
    <main className="flex-1 md:ml-64 p-margin-mobile md:p-margin-desktop w-full max-w-container-max mx-auto overflow-x-hidden pt-20 md:pt-margin-desktop bg-paper text-ink min-h-screen font-body-md">
      ...
      <header className="mb-stack-lg flex flex-col md:flex-row md:items-end justify-between gap-4 border-b-4 border-navy pb-4 animate-fade-in-up">
        <div>
        ...
        <div className="relative w-full md:w-48 shrink-0">
          <select value={selectedCity} onChange={e => setSelectedCity(e.target.value)} className="appearance-none w-full rounded-lg bg-surface p-3 pr-8 border border-border font-label-md text-sm focus:border-navy focus:ring-1 focus:ring-navy focus:outline-none text-ink shadow-sm cursor-pointer transition-colors hover:bg-paper">
      ...
      <div className="bg-surface border border-border rounded-xl shadow-sm overflow-hidden animate-fade-in-up" style={{ animationDelay: '100ms' }}>
        <div className="overflow-x-auto">
          <table className="w-full text-left font-body-md text-body-md">
            <thead className="bg-paper font-label-md text-label-md text-muted border-b border-border">
              <tr>
                <th className="py-4 px-6 font-bold uppercase tracking-widest text-[10px] w-16 text-center">Rank</th>
                <th className="py-4 px-6 font-bold uppercase tracking-widest text-[10px]">Citizen</th>
                <th className="py-4 px-6 font-bold uppercase tracking-widest text-[10px]">Ward</th>
                <th className="py-4 px-6 font-bold uppercase tracking-widest text-[10px] text-right">Civic Score</th>
                <th className="py-4 px-6 font-bold uppercase tracking-widest text-[10px] text-center">Badges</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                leaders.map((leader, index) => {
                  ...
                  return (
                    <tr key={leader.id} className={`hover:bg-paper transition-colors ${leader.id === user?.uid ? 'bg-navy/5 border-l-4 border-l-navy' : 'bg-surface'}`}>
